...
if not firebase_admin._apps:
    try:
        ...
        st.error("Error: No se encontró la clave 'firebase_service_account' en los secrets de Streamlit.")
        st.warning("Si pruebas localmente, asegúrate que 'service-account-key.json' esté en la carpeta.")
        st.stop()
    except ValueError as e:
        st.error(f"Error al leer las credenciales: {e}")
        st.warning("🚨 ¡Tu archivo 'service-account-key.json' parece estar corrupto o es inválido!")
        st.stop()
...
if df_logs.empty:
    ...
    if df_filtered.empty:
        ...
        # Definimos las columnas que SÍ queremos mostrar al usuario
        # Ocultamos 'firebase_id' y 'synced' que son de uso interno
        cols_to_show = ['timestamp', 'workerName', 'eventType', 'id']
        
        # Filtramos el dataframe para mostrar solo esas columnas (si existen)
